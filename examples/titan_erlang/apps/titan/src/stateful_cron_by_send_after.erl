...
cancel_weekend_stuff() ->
    gen_server:call(?MODULE, cancel_weekend_stuff).
...
handle_call(cancel_weekend_stuff, _From,
...
handle_info(weekend_stuff, State) ->
    do_weekend_stuff(),
    {ok, Ref} = send_after_weekend_msg(),
...
send_after_weekend_msg() ->
    ecron:send_after(?CrontabSpec, self(), weekend_stuff).

do_weekend_stuff() ->
    io:format("do weekday stuff at ~p~n", [erlang:localtime()]).
