...
prop_cron_apply_error() ->
    ?FORALL({Name, Request}, {name(), term()},
        begin
            ...
            timer:sleep(1100),
            {ok, #{crontab := CronSpec,
                start_time := {0, 0, 0}, end_time := {23, 59, 59},
                mfa := RMFA, name := RName,
                failed := Failed, ok := Ok,
                results := Result}} = ecron:statistic(ecron_local, Name),
            ok = ecron:delete(Name),
            error_logger:tty(true),
            CronSpec =:= 1 andalso
                RName =:= Name andalso
...
prop_singleton() ->
    ?FORALL({Name, Singleton}, {term(), bool()},
        begin
            ...
            #{start_time := {0, 0, 0}, end_time := {23, 59, 59}, status := activate,
...
prop_ecron_send_interval() ->
    ?FORALL({Message, NeedReg, Name}, {term(), bool(), atom()},
        begin
            ...
            {ok, Job1} = ecron:send_interval(ecron_local, make_ref(), "0 1 1 * * *", Message, unlimited, unlimited, []),
            error_logger:tty(true),
