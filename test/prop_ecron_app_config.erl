...
prop_application_ok_config() ->
    ?FORALL(
        {Spec, UseTime},
        {prop_ecron_spec:standard_spec(), bool()},
        ?IMPLIES(
            ...
            begin
                ...
                Jobs =
                    case UseTime of
                        true ->
                            [
                                {noraml, SpecStr, {io_lib, format, ["~s", [SpecStr]]}, unlimited,
                                    unlimited}
                            ];
                        false ->
                            [{normal, SpecStr, {io_lib, format, ["~s", SpecStr]}}]
                    end,
...
prop_application_error_config() ->
    ?FORALL(
        {Spec, UseTime},
        {prop_ecron_spec:maybe_error_spec(), range(0, 3)},
        begin
            error_logger:tty(false),
            application:start(telemetry),
            application:stop(ecron),
            ...
            Jobs =
                case UseTime of
                    0 ->
                        [{test, error_format}];
                    1 ->
                        [{test, SpecStr, {io_lib, format, ["~s", SpecStr]}}];
                    2 ->
                        [
                            {test, "* * * * * *", {io_lib, format, ["~s", SpecStr]}, {2019, 12, 10},
                                unlimited}
                        ];
                    3 ->
                        [{test, test, {io_lib, format, ["~s", SpecStr]}}]
                end,
            application:set_env(ecron, local_jobs, Jobs),
            ...
            Res =
                case T of
                    {error, {
                        {shutdown, {failed_to_start_child, ecron, Reason}},
                        ...
                        lists:member(Reason, [
                            ...
                            "invalid_spec: test"
                        ]) orelse check_spec(Reason, SpecStr);
                    ok ->
                        true
                end,
            error_logger:tty(true),
